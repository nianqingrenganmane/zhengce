  ...
  <div class="Details">
    ...
    <div class="box">
      ...
      <div class="time">
        ...
        <span>{{info.source?'来源：'+info.source:'政策解读'}}</span>
      </div>
      <div class="labelBox">
        <div class="label">
          <div class="labels labelys">政策解读</div>
          <div
            class="labels"
        ...
        <div class="mask" v-if="detailsy.header">
          <div class="maskLeft">
            <div class="maskLeftTit">政策速递</div>
            <div class="maskLeftNin">{{detailsy.header}}</div>
          </div>
          <div class="maskRight" @click="nextRouts(detailsy)">查看政策</div>
        </div>
      </div>
  ...
  methods: {
    ...
    detaInfo(id) {
      ...
      this.$axios.post("ent/info", daty).then(res => {
        ...
        this.detailsy = {
          header: res.data.data.policy_header,
          id: res.data.data.policy_id
        };
        ...
        this.$axios
          ...
          .then(res => {
            if (res.data.code == 20000) {
              ...
              this.$toast({
                message: "收藏失败，错误码：" + res.data.code,
                forbidClick: true,
                duration: 2000
              });
            }
